...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if (method === 'unique') {
            ...
            showMapLegend(
                ...
                uniques.slice(0, 15).map((v, i) => ({
                    color: QUAL_PALETTE[i % QUAL_PALETTE.length],
                    label: v,
                    shape: 'circle'
                }))
            );
    ...
    function updateVariableChart() {
        ...
        if (!layerId || !field || !layerState[layerId]?.geojsonData || !ctxVar) {
            ...
            if (emVar) emVar.classList.add('show');
            if (statsRow) statsRow.classList.add('hidden');
            return;
        }

        emVar.classList.remove('show');
